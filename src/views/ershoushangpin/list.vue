	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('ershoushangpin','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('ershoushangpin','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('ershoushangpin','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('ershoushangpin','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('ershoushangpin','查看')"
				:data="list"
				...
				<el-table-column label="商品图片" width="120" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('ershoushangpin','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('ershoushangpin','在线咨询')" type="success" @click="zaixianzixunCrossAddOrUpdateHandler(scope.row,'cross','','','')">在线咨询</el-button>
						<el-button v-if="btnAuth('ershoushangpin','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<zaixianzixunFormModel ref="zaixianzixunFormModelRef" @formModelChange="formModelChange"></zaixianzixunFormModel>
	</div>
</template>
<script setup>
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussershoushangpin?refid=' + id)
	}
	import zaixianzixunFormModel from '@/views/zaixianzixun/formModel'
	const zaixianzixunFormModelRef = ref(null)
    const zaixianzixunCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			zaixianzixunFormModelRef.value.init(row.id,'cross','在线咨询',row,'ershoushangpin',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
